...
while ($i -lt $n)
{
	...
	$Devices += @($Device.Substring(0,$Device.Length-2))
	echo $Devices[$i]
	$i++
}
...
while ($i -le $n)
{
	Rename-Item -Path D:\jenkins-slave\workspace\GHS-Android-Email_Notifier-Regression\Phone\Automation_report_*_$i.html -newname Regression_$i.html	
	$i++
}
...
while ($i -le $n)
{
	while ($j -lt $n)
	{
		$Filepath = "D:\jenkins-slave\workspace\GHS-Android-Email_Notifier-Regression\Phone\Regression_$i.html"
		$Filename = "Regression_$i"
...
Add-Content D:\jenkins-slave\workspace\GHS-Android-Email_Notifier-Regression\Phone\Regression.json "`n{"
